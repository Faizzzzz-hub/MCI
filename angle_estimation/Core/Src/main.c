...
// // float error = angle_filtered - setpoint;


// //         // angle deadband for STOP condition
// //         float deadband_angle = 2.0f;
...
// //             float u = (Kp*error + Ki*pid_integral + Kd*derivative) *5.0f; // scale as needed
// //             last_error = error;
...
//     int16_t gx_raw, gy_raw, gz_raw;
//     gyro_read_xyz(&gx_raw, &gy_raw, &gz_raw);
//     float gyro_raw = gy_raw * GYRO_SENS_245DPS;

//     // ----------- 2a) CALIBRATE GYRO BIAS -----------
//     if (first)
//     {
...
//         if (samples == 200)  // about 2 seconds
//         {
//             gyro_bias /= 200.0f;
//             first = 0;
//         }
//         return; // wait until calibration finishes
//     }

//     gyro_rate = gyro_raw - gyro_bias;
...
//     // ----------- 4) ERROR -----------
//     const float setpoint = -1.3f;   // robot upright is near 0°
// float error = angle_filtered - setpoint;   // <-- FIXED SIGN !!!
//     // ----------- 5) PID -----------
//     pid_integral += error * DT;
...
//     // minimum power so motors actually move
//     // if (pwm < 180) pwm = 180; og
...
//     // ----------- NEW SPEED BASED ON ANGLE (NOT PID POWER) -----------

// float absAngle = fabsf(error);
...
// if (absAngle < 1.0f)
// {
//     pwm = 0;                    // perfectly upright → no movement
// }
// else if (absAngle < 3.0f)
// {
//     pwm = 200;                  // small correction
// }
// else if (absAngle < 7.0f)
// {
//     pwm = 400;                  // medium correction
// }
// else
// {
//     pwm = 650;                  // large correction
// }
...
//     // if (u > 0)
//     // {
//     //     // tilt backward → move wheels backward
//     //     HAL_GPIO_WritePin(MOTOR_IN1_GPIO_PORT, MOTOR_IN1_PIN, GPIO_PIN_RESET);
//     //     HAL_GPIO_WritePin(MOTOR_IN2_GPIO_PORT, MOTOR_IN2_PIN, GPIO_PIN_SET);
...
//     // else
//     // {
//     //     // tilt forward → move wheels forward
//     //     HAL_GPIO_WritePin(MOTOR_IN1_GPIO_PORT, MOTOR_IN1_PIN, GPIO_PIN_SET);
//     //     HAL_GPIO_WritePin(MOTOR_IN2_GPIO_PORT, MOTOR_IN2_PIN, GPIO_PIN_RESET);
...
//     // ----------- 7) MOTOR DIRECTION (REVERSED) -----------
// if (u > 0)
// {
//     // tilt backward → wheels must go FORWARD to catch it
//     HAL_GPIO_WritePin(MOTOR_IN1_GPIO_PORT, MOTOR_IN1_PIN, GPIO_PIN_SET);
//     HAL_GPIO_WritePin(MOTOR_IN2_GPIO_PORT, MOTOR_IN2_PIN, GPIO_PIN_RESET);
...
// else
// {
//     // tilt forward → wheels must go BACKWARD to push under centre
//     HAL_GPIO_WritePin(MOTOR_IN1_GPIO_PORT, MOTOR_IN1_PIN, GPIO_PIN_RESET);
//     HAL_GPIO_WritePin(MOTOR_IN2_GPIO_PORT, MOTOR_IN2_PIN, GPIO_PIN_SET);
...
if (pid_integral < -20) pid_integral = -20;
            ...
            float derivative = (error - last_error) / DT;
            float u = (Kp*error + Ki*pid_integral + Kd*derivative)*14.0f;
            last_error = error;
